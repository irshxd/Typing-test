...
let textContainer = document.querySelector(".textcontainer p");
let inputfeild = document.querySelector(".someinput")
let mistakespan = document.querySelector(".mistake span")
let timerspan = document.querySelector(".time span")
let wpmspan = document.querySelector('.wpm span')
let accuracyspan = document.querySelector('.Cpm span')
...
function firstquote(){
    ...
    let quotespan = `<span>${span}</span>`; 
    textContainer.innerHTML += quotespan;
});
document.addEventListener('keydown',()=>inputfeild.focus());
textContainer.addEventListener("click",()=>inputfeild.focus());1
};
...
function inputtype (){
    ...
    if(typedchar==null){
        ...
        let accuracytext  = Math.floor(((wpm-mistakes)/wpm*100));
        accuracyspan.innerText=accuracytext;
    }else {
        clearInterval(timer);
    }
    }
...
firstquote();
inputfeild.addEventListener('input',inputtype);
